...
<head>
    ...
    <title>Edit Tip — PetParade Admin</title>
...
<div class="container">
    <div class="form-card">
        <h2><i class="fas fa-edit" style="color:#667eea;"></i> Edit Pet Tip</h2>
        ...
        <form action="{{ route('admin.tips.update', $tip->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <div class="pet-type-selector">
                    <label class="pet-option cat">
                        <input type="radio" name="pet_type" value="cat" {{ $tip->pet_type == 'cat' ? 'checked' : '' }} required>
                        <span class="emoji">🐱</span>
                        <span class="label">Cats</span>
                    </label>
                    <label class="pet-option dog">
                        <input type="radio" name="pet_type" value="dog" {{ $tip->pet_type == 'dog' ? 'checked' : '' }}>
                        <span class="emoji">🐶</span>
                        <span class="label">Dogs</span>
                    </label>
                    <label class="pet-option both">
                        <input type="radio" name="pet_type" value="both" {{ $tip->pet_type == 'both' ? 'checked' : '' }}>
                        <span class="emoji">🐾</span>
            ...
            <div class="form-group">
                ...
                <div class="category-selector">
                    <label class="cat-option nutrition">
                        <input type="radio" name="category" value="nutrition" {{ $tip->category == 'nutrition' ? 'checked' : '' }} required>
                        <i class="fas fa-carrot"></i> Nutrition
                    </label>
                    <label class="cat-option health">
                        <input type="radio" name="category" value="health" {{ $tip->category == 'health' ? 'checked' : '' }}>
                        <i class="fas fa-heartbeat"></i> Health
                    </label>
                    <label class="cat-option grooming">
                        <input type="radio" name="category" value="grooming" {{ $tip->category == 'grooming' ? 'checked' : '' }}>
                        <i class="fas fa-cut"></i> Grooming
                    </label>
                    <label class="cat-option training">
                        <input type="radio" name="category" value="training" {{ $tip->category == 'training' ? 'checked' : '' }}>
                        <i class="fas fa-graduation-cap"></i> Training
                    </label>
                    <label class="cat-option general">
                        <input type="radio" name="category" value="general" {{ $tip->category == 'general' ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input type="text" id="title" name="title" value="{{ old('title', $tip->title) }}" required>
            </div>

            {{-- Body --}}
            <div class="form-group">
                <label for="body">Tip Content *</label>
                <textarea id="body" name="body">{{ old('body', $tip->body) }}</textarea>
            </div>

            <div class="form-actions">
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-check"></i> Update Tip
                </button>
